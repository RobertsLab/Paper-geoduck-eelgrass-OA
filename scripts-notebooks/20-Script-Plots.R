...
pdf("results/Deployment-map.pdf", height=10, width=10)
...
# Fidalgo Bay
pH.FB.ga <- ggplot(data=subset(Env.Data.Master.noOuts.geo_daily, (metric=="pH" & (variable=="FB-E" | variable=="FB-B"))), aes(x=Day,y=daily.mean,colour=variable,group=variable)) + geom_line(size=2, aes(linetype=variable), color="royalblue3")  + scale_linetype_manual(values=c("dashed", "solid")) + geom_ribbon(aes(ymax=daily.mean + daily.sd, ymin=daily.mean-daily.sd, alpha=0.5), colour=NA, fill = "grey70") + theme_light() + theme(plot.title = element_text(size=24, face="bold", margin=margin(0,0,2,0)), axis.text.y=element_text(size=26, angle=45, face="bold"), axis.text.x=element_blank(), axis.title.x=element_blank(), axis.title.y=element_blank(), legend.position = "none", panel.background = element_blank()) + ggtitle("Daily mean pH differed between eelgrass (dotted)\n& unvegetated (solid) across estuarine bays") + ylim(6.78, 8.24) + labs(x="Date", y="pH", size=26) + theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + theme(plot.margin=unit(c(.2,.2,.5,0),"cm"), panel.border = element_rect(colour = "black", fill=NA, size=1))

#Port Gamble
pH.PG.ga <- ggplot(data=subset(Env.Data.Master.noOuts.geo_daily, (metric=="pH" & (variable=="PG-E" | variable=="PG-B"))), aes(x=Day,y=daily.mean,colour=variable,group=variable)) + geom_line(size=2, aes(linetype=variable), color="steelblue2")  + scale_linetype_manual(values=c("solid")) + geom_ribbon(aes(ymax=daily.mean + daily.sd, ymin=daily.mean-daily.sd, alpha=0.5), colour=NA, fill = "grey70") + theme_light() + theme(plot.title = element_blank(), axis.text.y=element_text(size=26, angle=45, face="bold"), axis.text.x=element_blank(), axis.title=element_blank(), legend.position = "none", panel.background = element_blank())  + ggtitle("Port Gamble Bay daily mean pH") + ylim(6.78, 8.24)  + theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + theme(plot.margin=unit(c(.2,.2,.5,0),"cm"), panel.border = element_rect(colour = "black", fill=NA, size=1))

#Case Inlet
pH.CI.ga <- ggplot(data=subset(Env.Data.Master.noOuts.geo_daily, (metric=="pH" & (variable=="CI-E" | variable=="CI-B"))), aes(x=Day,y=daily.mean,colour=variable,group=variable)) + geom_line(size=2, aes(linetype=variable), color="goldenrod1")   + scale_linetype_manual(values=c("dashed", "solid")) + geom_ribbon(aes(ymax=daily.mean + daily.sd, ymin=daily.mean-daily.sd, alpha=0.5), colour=NA, fill = "grey70") + theme_light() + theme(plot.title = element_blank(), axis.text.y=element_text(size=26, angle=45, face="bold"), axis.text.x=element_blank(), axis.title=element_blank(), legend.position = "none", panel.background = element_blank())  +  ggtitle("Case Inlet daily mean pH") + ylim(6.78, 8.24)  + labs(x="Date", y="pH", size=18) + theme(plot.margin=unit(c(.2,.2,.5,0),"cm"), panel.border = element_rect(colour = "black", fill=NA, size=1)) + theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank())

#Willapa Bay
pH.WB.ga <- ggplot(data=subset(Env.Data.Master.noOuts.geo_daily, (metric=="pH" & (variable=="WB-E" | variable=="WB-B"))), aes(x=Day,y=daily.mean,colour=variable,group=variable)) + geom_line(size=2, aes(linetype=variable), color="sienna1")   + scale_linetype_manual(values=c("dashed", "solid")) + geom_ribbon(aes(ymax=daily.mean + daily.sd, ymin=daily.mean-daily.sd, alpha=0.5), colour=NA, fill = "grey70") + theme_light() + theme(plot.title = element_blank(), axis.text.y=element_text(size=26, angle=45, face="bold"), axis.text.x=element_text(size=26, angle=45, hjust=0.95, vjust=0.9, face="bold"), axis.title.x=element_blank(), axis.title.y=element_blank(), legend.position = "none", panel.background = element_blank()) + labs(x="Date", y="pH", size=16) +  ggtitle("Willapa Bay daily mean pH") + ylim(6.78, 8.24) + theme(plot.margin=unit(c(.2,.2,.2,0),"cm"), panel.border = element_rect(colour = "black", fill=NA, size=1))

#jpeg(filename = "results/Environmental/pH-plots-graphical-abstract.jpeg", width = 620, height = 1200) 
pdf("results/Environmental/pH-plots-graphical-abstract.pdf", width = 9, height = 11) 
plot_grid(pH.FB.ga, pH.PG.ga, pH.CI.ga, pH.WB.ga,
          align = "v",   
          nrow=4,
          rel_heights = c(.6, .6, .6, .7)) #adjust row heights so plots in last row aren't smaller (they have x-axis tex
dev.off()
...
jpeg(filename = "results/SRM/all-peptide-graphical-abstract.jpeg", width = 550, height = 600) 
ggplot(data.melted.plus.promean, aes(x=factor(Sample.Shorthand, levels=levels(Sample.Shorthand)), y=value, color=Sample.Shorthand, fill=Sample.Shorthand)) + geom_violin(trim=F) + scale_color_manual(values=group.colors2) + scale_fill_manual(values=group.colors3) + theme_light() + theme(plot.title = element_text(size=24, face="bold", margin=margin(0,0,2,0)), axis.title.x=element_blank(), axis.text.x=element_blank(), axis.text.y=element_text(size=20, angle=45, hjust=0.95, vjust=0.9, face="bold"), axis.title=element_text(size=24,face="bold"), legend.position = "none", panel.background = element_blank()) + ggtitle("Protein and growth differed between\nbays (colors) but not between habitats\n(unvegetated = white, eelgrass = filled)") + labs(y="targeted peptide spectral abundance") + guides(fill = guide_legend(reverse = TRUE)) + theme(plot.margin=unit(c(.1,.1,.1,.1),"cm"), panel.border = element_rect(colour = "black", fill=NA, size=1)) + scale_y_continuous(labels=scales::scientific, breaks=c(0e6,2e6,4e6,6e6,8e6,10e6,12e6)) #+ coord_flip() 
dev.off()
