...
source("scripts-notebooks/BioStats.R") #Load via repo directory 
...
write.csv(SRM.reps4stats.plots, file="results/SRM/SRM-techrep-stats.csv") #Save tech rep stats data as .csv 
...
write.csv(SRM.reps4stats.s.plots, file="results/SRM/SRM-screened-techrep-stats.csv") #Save tech rep stats data as .csv 
...
# Number of proteins, peptides & transitions used in final data set for diff. analysis via ANOVA
nrow(unique(SRM.final[!(SRM.final$Protein %in% c("HSP70", "Peroxiredoxin-1", "Ras-related")), ]["Protein"])) #10 proteins in final data set
nrow(unique(SRM.final[!(SRM.final$Protein %in% c("HSP70", "Peroxiredoxin-1", "Ras-related")), ]["Peptide"])) #26 peptides in final data set
nrow(unique(SRM.final[!(SRM.final$Protein %in% c("HSP70", "Peroxiredoxin-1", "Ras-related")), ]["Transition"])) #77 transitions in final data set